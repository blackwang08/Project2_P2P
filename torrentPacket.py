...
class torrentPacket:
    ...
    @staticmethod
    def create_info(sender_id, info, upload, download):
        return torrentPacket(sender_id, 0, info, [upload, download])
    ...
    @staticmethod
    def create_req(sender_id, hashname, filename, chunk, upload, download):
        # create fake network data with only 1 file and 1 chunk to overload info field
        # informs receiver of what to send back
        reqData = torrentData()
        reqData.addPeer(sender_id, 0, 0, upload, download)
        reqData.addFile(hashname, filename, (chunk + 1) * torrentData.CHUNK_SIZE)
        reqData.peerAquireFileChunk(sender_id, hashname, chunk)
        return torrentPacket(sender_id, 2, reqData, [])

    @staticmethod
    def create_data(sender_id, hashname, filename, chunk, data, upload, download):
        # create fake network data with only 1 file and 1 chunk to overload info field
